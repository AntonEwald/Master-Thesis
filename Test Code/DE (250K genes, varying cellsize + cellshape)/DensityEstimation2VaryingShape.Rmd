...
cbind(1:nrow(gg), gg[order(gg[,4]), ])[1:10, 5]

cbind(1:nrow(gg), gg[order(gg[,j+1]), ])[1:10, 253]
...
Find estimated modes
```{r}
nr_of_clusters = length(unique(GMM_data_VS$Samples))
Estimated_modes1_VS <- matrix(NA, nrow = nr_of_clusters, ncol = 6)
Estimated_modes2_VS <- matrix(NA, nrow = nr_of_clusters, ncol = 6)
Estimated_modes3_VS <- matrix(NA, nrow = nr_of_clusters, ncol = 6)

tt <- proc.time()
for (i in 1:10) {
  M <- cbind(1:nrow(GMM_coordinates_VS), apply(GMM_coordinates_VS, MARGIN = 1, FUN = function(x) sqrt(sum((x[]-mu_VS[i,])^2))), rho1_VS, rho2_VS, kde_dens_VS)
  ordered <- M[order(M[,2]),] %>% .[1:200,] %>% 
    cbind(1:nrow(.))
...
Faster loop but needs a N*k matrix (N = nr of data, k = nr of clusters). Can be hard for memory with very large data. Then above code works
```{r}
gg <- cbind(1:nrow(GMM_coordinates_VS), t(dista(mu_VS, GMM_coordinates_VS)), rho1_VS, rho2_VS, kde_dens_VS)
nr_of_clusters = length(unique(GMM_data_VS$Samples))
Estimated_modes1_VSb <- matrix(NA, nrow = nr_of_clusters, ncol = 4)
Estimated_modes2_VSb <- matrix(NA, nrow = nr_of_clusters, ncol = 4)
Estimated_modes3_VSb <- matrix(NA, nrow = nr_of_clusters, ncol = 4)
tt2 <- proc.time()
for (j in 1:10){
a <- cbind(1:200, gg[order(gg[,j+1]), ][1:200, ])

Estimated_modes1_VSb[j, ] = a[which.max(a[, 253]), c(1, 2, j+2, 253)]
Estimated_modes2_VSb[j, ] = a[which.max(a[, 254]), c(1, 2, j+2, 254)]
Estimated_modes3_VSb[j, ] = a[which.max(a[, 255]), c(1, 2, j+2, 255)]
print(j)
}
time2 <- proc.time() - tt2
```
